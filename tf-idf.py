...
##loop to remove puntuation and extra spaces
...
#Looping through the histogram to count the amount of times a word appears on the document
for data in dataset:
    words = nltk.word_tokenize(data)
    for word in words:
        if word not in words2count.keys():
            words2count[word] = 1
        else:
            words2count[word] += 1

#getting the 100 most frequent words
freq_words = heapq.nlargest(100,words2count,key=words2count.get)
...
#multiplying matrixes to obtain tf-idf value
...
X = np.transpose(np.asarray(tfidf_matrix))
